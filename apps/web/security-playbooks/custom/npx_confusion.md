...
There is no CVE and no vulnerable installed version here, so this does not go through `finding_add`; that tool requires an advisory-matched CVE. Use `finding_add` only after the applicable core condition is fully verified.
