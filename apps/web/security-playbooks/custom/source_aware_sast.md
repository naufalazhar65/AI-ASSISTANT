...
cache, and registry configuration. `security_playbook name="npx_confusion"` when a bare
...
dynamic validation" rule below: report each one with `finding_add` (kategori
OWASP A06) or `dep_audit to_findings=true`, setting `cvss` from the published
advisory. Use `verify_patch` to track which are actually fixed.
