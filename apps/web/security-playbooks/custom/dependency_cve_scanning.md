...
description: Supply-chain / SCA playbook — scan repository lockfiles for known dependency CVEs and report them with finding_add (no dynamic PoC required)
---
...
the dedicated `finding_add` tool.
...
dynamic-validation rule, and it exists only for `finding_add`.
...
per manifest. File one `finding_add` per CVE — do not batch
...
contains it, then pass to `finding_add`:
...
`cvss` alone); it exists so the reader can prioritize.
...
vulnerable code path is reachable. Report it, set `cvss` from the
...
as a normal dynamic finding with `finding_add` (the standalone
CVE stays in its own `finding_add`).

## Reporting

Report each confirmed known CVE with the dedicated `finding_add`
tool (NOT `finding_add` — that tool is for dynamically validated
...
- Set `cve` to the verified `CVE-YYYY-NNNNN` id (required). If you only have a
  ...
  with this tool.
- There are no PoC fields — `finding_add` does not take
...
- **Always set `cvss` to the published advisory base score (0.0–10.0).**
...
the finding's severity. `cvss` stays the published reference.
...
never replaces `cvss` as the published reference.
...
- Do not report a dependency CVE with `finding_add`; use
  `finding_add`.
- Do not report a finding without a verified CVE id.
- Do not batch multiple CVEs into one report.
- Do not omit `cvss` — the tool rejects it, and it rates every finding
