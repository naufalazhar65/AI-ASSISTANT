...
reachable. File it with `finding_add`.
...
is a legitimate, expected outcome. Catat sebagai **NEEDS_FOLLOW_UP**
(cantumkan item terbuka di bagian akhir laporan), dan jelaskan gap-nya di
kolom evidence/impact saat menyusun laporan (`report_generate`). Jangan
konversi ke `ruled_out` hanya untuk merapikan daftar kerja.
...
Closure is only useful if it is written down. Mia tidak punya ledger
cakupan terpisah — tulis status penutupan setiap permukaan yang diuji di
laporan (`report_generate`), dan simpan detail kontrol/pengujian di kolom
`evidence` temuan. Setiap permukaan harus berakhir di salah satu status:
...
they lacked, update that surface's record rather than recording a
parallel one. This runs both ways — a `ruled_out` whose
