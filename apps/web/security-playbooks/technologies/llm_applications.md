...
- Use `finding_add` (kategori OWASP A06, atau `dep_audit to_findings=true`) only for verified advisory-matched dependency CVEs. Use `finding_add` for dynamically verified application, model, RAG, agent, or supply-chain findings.
