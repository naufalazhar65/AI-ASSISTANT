...
const toolRegistry: ToolPlugin[] = [
  ...
  {
    definition: { type: "function", risk: "write", function: { name: "cdp_proxy", description: "Mini-proxy: rekam TRAFFIK LIVE dari tab Chrome user (patch fetch/XHR in-page, pola tamper) selama N detik — menemukan endpoint yang hanya muncul saat runtime (API pasca-login, GraphQL, polling) tanpa install sertifikat/proxy. NILAI query/body TIDAK direkam (hanya NAMA param — rahasia tetap di browser), host out-of-scope disembunyikan, hasil otomatis masuk target_brain. WAJIB user berinteraksi dengan halaman selama perekaman. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { tab: { type: "string", description: "potongan URL tab target" }, seconds: { type: "number", description: "durasi rekam 5-120 (default 30)" }, brain: { type: "boolean", description: "catat ke target_brain (default true)" } }, required: ["tab"] } } },
    execute: async (args, ctx) => { try { const { cdpProxy } = await import("./cdpProxy"); return await cdpProxy(ctx?.rawUser, { tab: String(args.tab || ""), seconds: typeof args.seconds === "number" ? args.seconds : undefined, brain: args.brain !== false }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "cdp_proxy failed"}`; } },
  },
  {
    definition: {
      type: "function",
  ...
  {
    definition: { type: "function", risk: "write", function: { name: "bypass403", description: "403/401 access-control bypass matrix: baseline dulu (URL harus memang ditolak), lalu trik klasik (double slash, %2e, /..;/, trailing . dan ;, X-Original-URL/X-Rewrite-URL, IP-loopback headers, verb override, _method body, Host localhost) — verdict host-based: 2xx + body BEDA dari halaman deny = LEAD (SPA catch-all yang body-nya sama TIDAK dihitung). Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "URL yang menjawab 403/401" }, session: { type: "string", description: "nama http_session (opsional)" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { bypass403 } = await import("./bypass403"); return await bypass403(ctx.rawUser, { url: String(args.url || ""), session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "bypass403 failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "otp_probe", description: "OTP/2FA rate-limit & oracle prover: 1 baseline + N kode SALAH (bounded ≤15, BUKAN brute) → klasifikasi jujur: no-rate-limit (tanpa 429/lockout), oracle pesan (invalid vs expired), respons success-like; `samples` (kode OTP milikmu sendiri) menghitung entropy code-space (6 digit ≈ brute-able tanpa rate-limit). Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint verifikasi kode" }, method: { type: "string" }, param: { type: "string", description: "nama field kode (default code)" }, body_template: { type: "string", description: "template body dgn marker {OTP}" }, session: { type: "string" }, attempts: { type: "number", description: "3-15 (default 8)" }, samples: { type: "array", description: "kode asli milikmu sendiri untuk hitung entropy" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { otpProbe } = await import("./otpProbe"); return await otpProbe(ctx.rawUser, { url: String(args.url || ""), method: typeof args.method === "string" ? args.method : undefined, param: typeof args.param === "string" ? args.param : undefined, body_template: typeof args.body_template === "string" ? args.body_template : undefined, session: typeof args.session === "string" ? args.session : undefined, attempts: typeof args.attempts === "number" ? args.attempts : undefined, samples: Array.isArray(args.samples) ? args.samples.map(String) : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "otp_probe failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "proto_pollute", description: "Prototype pollution prover: payload __proto__/constructor.prototype via query + JSON body (server) → STRONG bila marker muncul di respons / re-fetch bersih (persisten), WEAK bila error menyebut __proto__ atau 500 baru; + deteksi client gadget (source location/postMessage → merge/assign sink) di JS halaman. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang menerima input (lab/engagement)" }, param: { type: "string", description: "nama key marker (default polluted)" }, session: { type: "string" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { protoPollute } = await import("./protoPollute"); return await protoPollute(ctx.rawUser, { url: String(args.url || ""), param: typeof args.param === "string" ? args.param : undefined, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "proto_pollute failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "cache_decep", description: "Web Cache Deception prover (CWE-525): auth'd fetch varian decoy (.css/.js//test.css/;%23) pada path TERPROTEKSI, lalu re-fetch ANONIM — anon yang menerima konten terproteksi = lead cache deception. SPA catch-all ditanqani jujur (bukan temuan). Verifikasi browser anonim + poc_verify sebelum finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "path TERPROTEKSI (mis. https://host/account)" }, session: { type: "string", description: "nama http_session (auth'd step; tanpa session hanya membuktikan cacheability)" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { cacheDecep } = await import("./cacheDecep"); return await cacheDecep(ctx.rawUser, { url: String(args.url || ""), session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "cache_decep failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "nosql_hunt", description: "NoSQL injection prover (CWE-943): operator $ne/$gt/$regex + nested-object syntax pada field login/query (POST JSON) → outcome BEDA dari baseline kredensial-salah = auth-bypass LEAD; error Mongo/BSON = fingerprint info. Bounded ≤12 request. Verifikasi via poc_verify sebelum finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint login/query (POST JSON)" }, fields: { type: "string", description: "nama field dipisah koma (default username,password)" }, session: { type: "string" }, method: { type: "string", description: "default POST" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { nosqlHunt } = await import("./nosqlHunt"); return await nosqlHunt(ctx.rawUser, { url: String(args.url || ""), fields: typeof args.fields === "string" ? args.fields : undefined, session: typeof args.session === "string" ? args.session : undefined, method: typeof args.method === "string" ? args.method : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "nosql_hunt failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "path_traversal", description: "Path traversal / LFI prover (CWE-22/98/73): two-stage read-marker — satu payload traversal langsung per param vs baseline bersih, lalu baterai encoder PENUH hanya pada param responsif (%2f, dot-folding ....//, backslash Windows, absolute path, php://filter base64). Marker: /etc/passwd (root:x:0:0:), win.ini ([fonts]), php source (<?php) — WAJIB ABSEN dari baseline (halaman yang memang menampilkan passwd bukan bukti). RFI via callback https (reflection = info, bukti hanya via oast_poll). Bounded ≤4 param × ≤8 payload. Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang menerima param file/path (lab/engagement)" }, params: { type: "string", description: "param koma-terpisah (default: 20 nama file-ish umum, dipakai ≤4)" }, session: { type: "string", description: "nama http_session" }, callback: { type: "string", description: "OAST callback https untuk RFI (opsional)" }, method: { type: "string", description: "GET (default) atau POST" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { pathTraversal } = await import("./pathTraversal"); return await pathTraversal(ctx.rawUser, { url: String(args.url || ""), params: typeof args.params === "string" ? args.params : undefined, session: typeof args.session === "string" ? args.session : undefined, callback: typeof args.callback === "string" ? args.callback : undefined, method: typeof args.method === "string" ? args.method : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "path_traversal failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "otp_hunt", description: "2FA/OTP bypass suite (CWE-308/640/287): LEAK (kode OTP muncul di respons challenge di luar input-echo — json key/header/body), REUSE/STATELESS (kode salah sama diterima dua kali ≠ baseline deny — verifier tidak membakar/memvalidasi), CROSS-ACCOUNT (verifier menerima kode untuk identitas berbeda — binding kode↔akun lemah), FIELD-CONFUSION (field alternatif diterima seperti field utama). Baseline-controlled (kode salah deterministik — BUKAN brute), bounded ≤14 request. Melengkapi otp_probe (rate/oracle/entropy). Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint verifikasi OTP (lab/engagement)" }, request_url: { type: "string", description: "endpoint challenge/kirim kode (default: url)" }, method: { type: "string", description: "POST (default) atau GET" }, placement: { type: "string", enum: ["form", "json", "query"], description: "default form" }, code_field: { type: "string", description: "default code" }, user_field: { type: "string", description: "default username" }, user_value: { type: "string", description: "identitas akun A (WAJIB)" }, pass_field: { type: "string", description: "field password opsional" }, pass_value: { type: "string", description: "password opsional" }, user_value_b: { type: "string", description: "identitas akun B (untuk cross-account)" }, field_map: { type: "string", description: "field kode alternatif koma-terpisah (maks 3)" }, session: { type: "string", description: "nama http_session" } }, required: ["url", "user_value"] } } },
    execute: async (args, ctx) => { try { const { otpHunt } = await import("./otpHunt"); return await otpHunt(ctx.rawUser, { url: String(args.url || ""), request_url: typeof args.request_url === "string" ? args.request_url : undefined, method: typeof args.method === "string" ? args.method : undefined, placement: typeof args.placement === "string" ? args.placement : undefined, code_field: typeof args.code_field === "string" ? args.code_field : undefined, user_field: typeof args.user_field === "string" ? args.user_field : undefined, user_value: typeof args.user_value === "string" ? args.user_value : undefined, pass_field: typeof args.pass_field === "string" ? args.pass_field : undefined, pass_value: typeof args.pass_value === "string" ? args.pass_value : undefined, user_value_b: typeof args.user_value_b === "string" ? args.user_value_b : undefined, field_map: typeof args.field_map === "string" ? args.field_map : undefined, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "otp_hunt failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "account_recovery", description: "Password-recovery ATO prover (CWE-640/641/204/330): HOST-INJECTION (Host/X-Forwarded-Host kami diikuti reset link di respons — link email ber-host attacker = ATO by click; base URL berubah tanpa injeksi juga dilaporkan), TOKEN-PREDICTABLE (dua reset berturut: token identik / prefix panjang timestamp-like / alphabet kecil = guessable), USER-ENUMERATION (respons existing vs non-existing IDENTIK di forgot + login differential). Email TIDAK pernah dikirim/dibaca tool ini — klaim ATO butuh bukti klik/manual. Bounded ≤12 request. Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { request_url: { type: "string", description: "endpoint forgot-password (POST identitas akun)" }, account: { type: "string", description: "identitas akun target (default owner@test.local)" }, fake_account: { type: "string", description: "identitas non-existing sebagai kontrol (default nosuchuser.invalid)" }, account_field: { type: "string", description: "default email" }, login_url: { type: "string", description: "endpoint login opsional untuk differential enum" }, host_header: { type: "string", description: "host attacker (default evil.example)" }, placement: { type: "string", enum: ["form", "json"], description: "default form" }, session: { type: "string", description: "nama http_session" } }, required: ["request_url"] } } },
    execute: async (args, ctx) => { try { const { accountRecovery } = await import("./accountRecovery"); return await accountRecovery(ctx.rawUser, { request_url: String(args.request_url || ""), account: typeof args.account === "string" ? args.account : undefined, fake_account: typeof args.fake_account === "string" ? args.fake_account : undefined, account_field: typeof args.account_field === "string" ? args.account_field : undefined, login_url: typeof args.login_url === "string" ? args.login_url : undefined, host_header: typeof args.host_header === "string" ? args.host_header : undefined, placement: typeof args.placement === "string" ? args.placement : undefined, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "account_recovery failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "csv_inject", description: "CSV/formula injection prover (CWE-1236): simpan formula (=cmd DDE / =HYPERLINK exfil / prefix @,+ / tab) ber-marker unik via endpoint store, lalu export dicek MENTAH vs di-sanitasi — RAW di artifact spreadsheet (csv/tsv/xlsx/ods) = LEAD; sanitized = defense bekerja (negatif jujur); marker tidak sampai export = hop putus (jujur). Dua hop WAJIB teramati; tool TIDAK membuka file — klik korban di luar bukti otomatis. Bounded ≤8 request. Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang MENYIMPAN nilai (create/update, lab/engagement)" }, method: { type: "string", description: "POST (default) atau GET" }, field: { type: "string", description: "field penerima payload (default name)" }, extra_fields: { type: "string", description: "k=v koma-terpisah, ikut tiap store request" }, export_url: { type: "string", description: "endpoint export eksplisit (default: 8 path umum diprobe)" }, placement: { type: "string", enum: ["form", "json"], description: "default form" }, session: { type: "string", description: "nama http_session" }, payload: { type: "string", description: "formula kustom (default DDE)" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { csvInject } = await import("./csvInject"); return await csvInject(ctx.rawUser, { url: String(args.url || ""), method: typeof args.method === "string" ? args.method : undefined, field: typeof args.field === "string" ? args.field : undefined, extra_fields: typeof args.extra_fields === "string" ? args.extra_fields : undefined, export_url: typeof args.export_url === "string" ? args.export_url : undefined, placement: typeof args.placement === "string" ? args.placement : undefined, session: typeof args.session === "string" ? args.session : undefined, payload: typeof args.payload === "string" ? args.payload : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "csv_inject failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "blind_cmdi", description: "Blind command injection prover (CWE-78/77) — untuk endpoint yang TIDAK menampilkan output shell: OAST (payload ;curl <canary> per-param, CONFIRMED hanya via oast_poll hit ter-atribusi — refleksi string BUKAN bukti) + TIME-BASED (;sleep 6 dengan baseline pair — delta harus ≥5s dan >3× jitter baseline). Auto-create OAST callback bila tidak diberikan; time_only=1 untuk target tanpa egress. Bounded ≤10 request. Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang mem-fetch input ke shell (converter/ping/dns, lab/engagement)" }, method: { type: "string", description: "GET (default) atau POST" }, params: { type: "string", description: "param koma-terpisah (default 18 nama file/host-ish, dipakai ≤4)" }, callback: { type: "string", description: "OAST callback https (opsional; auto-create bila kosong)" }, time_only: { type: "boolean", description: "true = skip OAST (tanpa egress)" }, session: { type: "string", description: "nama http_session" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { blindCmdi } = await import("./blindCmdi"); return await blindCmdi(ctx.rawUser, { url: String(args.url || ""), method: typeof args.method === "string" ? args.method : undefined, params: typeof args.params === "string" ? args.params : undefined, callback: typeof args.callback === "string" ? args.callback : undefined, time_only: args.time_only === true, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "blind_cmdi failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "ssti_enum", description: "SSTI engine-fingerprint prover (CWE-1336): ladder aritmetika ({{7*7}}/${7*7}/#{7*7}/<%= 7*7 %>) + TELLER {{7*'7'}} (7777777 = Jinja2 string-repeat vs 49 = Twig) + error-shape fingerprint (Twig/Freemarker/Velocity/Pebble/Thymeleaf/ERB/EJS). Output: ENGINE + confidence (confirmed/likely/shape-only) + langkah lanjut AMAN (playbook pointer) — tool TIDAK mengirim payload RCE. Baseline rule: 49 di halaman baseline = param lain. Bounded ≤9 request. Sinyal → poc_verify → finding_add. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang me-render input ke template (lab/engagement)" }, param: { type: "string", description: "default name" }, method: { type: "string", description: "GET (default) atau POST" }, session: { type: "string", description: "nama http_session" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { sstiEnum } = await import("./sstiEnum"); return await sstiEnum(ctx.rawUser, { url: String(args.url || ""), param: typeof args.param === "string" ? args.param : undefined, method: typeof args.method === "string" ? args.method : undefined, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "ssti_enum failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "param_miner", description: "Unkeyed param/header discovery (Param-Miner-style, CWE-471/288): baseline pair → tiap kandidat di-replay SATU per SATU → LEAD hanya bila respons berbeda dari KEDUA baseline dengan arah yang baseline tidak pernah bedakan (status / body >60 byte & >2× jitter / header baru). Kandidat: 19 param tersembunyi (debug/admin/internal/role/...) + 10 header (X-Original-URL/X-Forwarded-Host/X-Debug/...). Kandidat yang mengubah respons pada halaman cacheable = material cache-unkeying (pointer ke cache_poison_prover, bukan klaim). LEAD ≠ vuln — makna diuji lanjut. Bounded ≤26 request. Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "halaman yang dihunt input tersembunyinya (lab/engagement)" }, params: { type: "string", description: "kandidat param koma-terpisah (default 19, dipakai ≤12)" }, headers: { type: "string", description: "kandidat header koma-terpisah (default 10, dipakai ≤10)" }, cache_note: { type: "boolean", description: "default true — sertakan pointer cache-unkeying" }, session: { type: "string", description: "nama http_session" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { paramMiner } = await import("./paramMiner"); return await paramMiner(ctx.rawUser, { url: String(args.url || ""), params: typeof args.params === "string" ? args.params : undefined, headers: typeof args.headers === "string" ? args.headers : undefined, cache_note: args.cache_note !== false, session: typeof args.session === "string" ? args.session : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "param_miner failed"}`; } },
  },
  {
    definition: { type: "function", risk: "write", function: { name: "blind_ssrf", description: "Blind-SSRF via DNS-callback (CWE-918): canary DNS ter-atribusi per-param (label p<idx>-<param> di subdomain) dari oast_dns (interactsh) → satu poll menyebut param MANA yang memicu resolve. Menangkap SSRF yang egress HTTP-nya diblok (DNS-only). Butuh interactsh-client (oast_dns_create). Scope-gated. Write, confirm.", parameters: { type: "object", properties: { url: { type: "string", description: "endpoint yang menerima URL/host (mis. /api/fetch?url=…)" }, params: { type: "string", description: "param koma-terpisah (default 18 nama umum, dipakai ≤8)" }, session: { type: "string" }, seconds: { type: "number", description: "tunggu callback 8-60 (default 20)" } }, required: ["url"] } } },
    execute: async (args, ctx) => { try { const { blindSsrf } = await import("./blindSsrf"); return await blindSsrf(ctx.rawUser, { url: String(args.url || ""), params: typeof args.params === "string" ? args.params : undefined, session: typeof args.session === "string" ? args.session : undefined, seconds: typeof args.seconds === "number" ? args.seconds : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "blind_ssrf failed"}`; } },
  },
  {
    definition: { type: "function", risk: "read", function: { name: "dns_audit", description: "Audit posture DNS (read-only): AXFR zone-transfer ke NS domain (dump = temuan instan), DNSSEC (DNSKEY), CAA (issuance policy). Keyless via dig; any-domain seperti recon_subdomains. Read, auto.", parameters: { type: "object", properties: { domain: { type: "string", description: "domain tanpa scheme/path (mis. example.com)" } }, required: ["domain"] } } },
    execute: async (args, ctx) => { try { const { dnsAudit } = await import("./dnsAudit"); return await dnsAudit(ctx?.rawUser, { domain: String(args.domain || "") }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "dns_audit failed"}`; } },
  },
  {
    definition: { type: "function", risk: "read", function: { name: "oast_dns", description: "Kelola DNS-OAST interactsh (keyless): tanpa argumen = status/domain aktif; action=create membuat domain canary unik; action=poll membaca interaksi DNS/HTTP; action=stop menghentikan. Pakai untuk bukti OOB blind (SSRF DNS-only, XXE, log4j). Read, auto.", parameters: { type: "object", properties: { action: { type: "string", enum: ["create", "poll", "stop"] } }, required: [] } } },
    execute: async (args, ctx) => { try { const { oastDnsCreate, oastDnsPoll, oastDnsStop } = await import("./oastDns"); const u = ctx?.rawUser; const a = String(args.action || ""); if (a === "create") return await oastDnsCreate(u); if (a === "stop") return await oastDnsStop(u); if (a === "poll") return await oastDnsPoll(u); const st = await import("./oastDns"); void st; return "🧬 DNS-OAST: pakai action=create (domain canary), action=poll (cek interaksi), action=stop. Create dulu sebelum blind_ssrf/payload OOB."; } catch (e) { return `Error: ${e instanceof Error ? e.message : "oast_dns failed"}`; } },
  },
  {
  ...
  {
    definition: { type: "function", risk: "read", function: { name: "jwt_attack", description: "Toolkit JWT: decode, forge alg:none, HS256 (secret), alg-confusion (public key), crack secret HS256 lemah, kid-injection (traversal ke /dev/null), jku-injection (JWKS attacker). Lokal (tanpa jaringan). Read, auto. Uji token hasilnya via http_request ke target berizin.", parameters: { type: "object", properties: { action: { type: "string", enum: ["decode", "none", "hs256", "confusion", "crack", "kid", "jku"] }, token: { "type": "string" }, secret: { type: "string" }, publicKey: { type: "string", description: "PEM kunci publik server (confusion) / URL JWKS attacker (jku)" }, claims: { type: "string", description: "JSON claim override, mis. {\"role\":\"admin\"}" }, words: { type: "string", description: "kata tambahan untuk crack" } }, required: ["action"] } } },
