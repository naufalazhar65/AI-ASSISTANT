...
const toolRegistry: ToolPlugin[] = [
  ...
  },
  {
    ...
    execute: async (args, ctx) => { try { const { raceAttack } = await import("./attack"); return await raceAttack(ctx.rawUser, { url: String(args.url || ""), method: typeof args.method === "string" ? args.method : undefined, body: typeof args.body === "string" ? args.body : undefined, headers: args.headers && typeof args.headers === "object" ? (args.headers as Record<string, string>) : undefined, count: typeof args.count === "number" ? args.count : undefined }); } catch (e) { return `Error: ${e instanceof Error ? e.message : "race failed"}`; } },
  },
  {
