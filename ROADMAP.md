...
[x] Brain upgrade: OpenCode Go (GLM 5.2) sebagai otak default  — provider `opencodego` (gateway OpenAI-compatible `https://opencode.ai/zen/go/v1/chat/completions`, $10/mo, 17 model); key dari env `OPENCODEGO_API_KEY` atau login opencode CLI (`serverKeys.ts ensureOpenCodeGoKey()` — providers.ts tetap client-safe); header wajib `x-opencode-session` (stabil per-user) + User-Agent `mia-assistant/1.0`; field custom `tools[].risk` TIDAK dikirim ke LLM (gateway strict menolak extra field); default di-.env.local untuk web/Telegram/Discord (voice tetap Groq). Live: persona 3.2s, tool 5s benar (2026-09-07)
