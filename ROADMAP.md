...
[x] Real-world place honesty guard (anti-halusinasi venue tutup)  — model (terutama 9router) suka bebas-asosiasi daftar kafe/tempat dari data training basi ("Arah Coffee"/"Kopi Kalyan" yang sudah tutup di Tangsel) meski system prompt sudah disuruh verifikasi. Solusi DETERMINISTIK: `lib/placeIntent.ts` `detectPlaceIntent` (rekomendasi tempat + status buka/tutup) & `placeNudge` (suffix jujur) + `agent.ts` `schedulePlaceCheckFromIntent` di kedua branch (groq/9router & opencode): kalau turn minta rekomendasi/status tempat DAN tidak web_search sukses, append " (Catatan: ini rekomendasi dari ingatanku dan bisa telat — cek dulu di Google ya, siapa tau ada yang udah tutup atau pindah 🌸)" — tidak pernah nge-lab saat model sudah hedged sendiri atau sudah web_search sukses (cuma web_search galat = tetap nudge; dilacak via `collector.webSearchSuccess` di `runAgent`). verify.ts `place honesty guard: OK`; LIVE 9router: rekomendasi tanpa verifikasi → nudge muncul; jawaban yang sudah "cek Google Maps" → tidak dobel nudge (2026-09-07)
[x] Brain upgrade: OpenCode Go (GLM 5.2) sebagai otak default  — provider `opencodego` (gateway OpenAI-compatible `https://opencode.ai/zen/go/v1/chat/completions`, $10/mo, 17 model); key dari env `OPENCODEGO_API_KEY` atau login opencode CLI (`serverKeys.ts ensureOpenCodeGoKey()` — providers.ts tetap client-safe); header wajib `x-opencode-session` (stabil per-user) + User-Agent `mia-assistant/1.0`; field custom `tools[].risk` TIDAK dikirim ke LLM (gateway strict menolak extra field); default di-.env.local untuk web/Telegram/Discord (voice tetap Groq). Live: persona 3.2s, tool 5s benar (2026-09-07); model default via `.env.local` `OPENCODEGO_MODEL` kini `deepseek-v4.1-flash` (spec default `glm-5.2`) — 2026-09-14. **Catatan 2026-09-21:** kuota bulanan Go HABIS (`429 GoUsageLimitError`, reset ~15 hari / top-up) → default web/Telegram/webhook pindah ke `9router` (64-window chain analisis); `opencodego` tetap full-window (semua 300 tool) dan dipulihkan begitu kuota reset
[x] Codebase QA — Mia paham kode proyek  — `codebaseIndex.ts` (repo + ALLOWED_WORKSPACES → chunk ~80 baris → BM25 shared `bm25Scores` dengan rag.ts; bounded 15s/4000 file/12000 chunk; deny .env/secrets/node_modules; persist `.data/codebase-index.json` + globalThis; auto-rebuild >24h) + tools `codebase_search`/`codebase_refresh` (read, auto); `MAX_TOOL_ROUNDS` 3→5 (model chain search→read→exec sampai "too many tool rounds"); snippet 1200 char + panduan anti-chain di deskripsi tool. Live: "dedup reminder di mana?" → reminders.ts addReminder ±141-220 akurat (2026-09-07)
[x] Weekly Insight (rekap mingguan)  — `weeklyInsight.ts`: digest 7 hari (mood label Indonesia + kalimat empatik, task incl. overdue, tema percakapan dihitung per HARI BERBEDA ≥2 hari supaya burst 1 malam tidak dominan); template deterministik + opener/closer bervariasi; push otomatis tiap Minggu 20:00 (`WEEKLY_INSIGHT_HOUR/DAY`, 0=off) + tool `weekly_insight`; dedup tahan restart (`.data/weekly-insight-state.json` atomic — pelajaran bug dobel recap diterapkan sejak awal); junk-filtered, silent tanpa data (2026-09-07)
[x] Output quality guards (mood/greeting telegrafik, prose tool call, clock ID)  — `stripToolCallProse()` strip prose tool-call (`mood_log(...)`, inline sekalipun) SEBELUM post-processor (prose "remind" mensupresi suffix lalu ter-strip → reply kosong); `ensureMoodReplyQuality` rewrite telegrafik (semua kalimat ≤4 kata) di turn mood/sapaan dengan empati hangat rotasi harian (contoh BAD di prompt tidak boleh dikutip — ditiru model); "jam 1 siang"=13:00 + bare clock "jam 8 pas" → `nearestOccurrence` (malam → 20:00, bukan besok pagi); fallback "…" → pesan hangat (2026-09-07)
...
[x] Node/device — macOS/iOS/Android (camera, location, device command) — `devices.ts` minimal safe macOS node (per-user `devices.json`, `DEVICE_SECRET` pairing, `device_list`/`device_exec`/`device_screenshot` tools, `ensureLocalDevice` auto local-mac, `POST /api/devices`); verify `device: OK` (2026-09-04); **ekstensi 2026-09-07:** `device_exec` + `blueutil -p [0|1]` (Bluetooth off/on/status, validasi arg ketat, butuh `brew install blueutil` — sudah terpasang); **monitor Mac** kind "device" (batre ≤ ambang / storage ≥ ambang, inklusif di ambang; metrik `pmset`/`df` volume data `/System/Volumes/Data`); iOS/Android eksekusi lebih dalam masih gap
...
[x] Reminder honest delivery audit (2026-09-14) — listener radio-ack (`ReminderListener` → boolean): slot hanya terkirim kalau benar-benar ada kanal yang push; tanpa ack (laptop off / kanal belum terekam) slot TIDAK dibakar — tercatat `missedAt`+`delivered:false`, retry saat connect berikutnya. Daily acked → reschedule +24h (`lastFiredAt`/`deliveredAt`); gap `deliveredAt − lastFiredAt` >30m → "kesampaian TELAT … pas device off". `reminders_list`/`buildReminderList` cetak `terkirim`/`KELEWAT`/`TELAT`; REMINDER HONESTY prompt wajibkan akui kelewat + tawarkan resend. SSE subscribe-dulu-replay. Exactly-once per slot terkirim. Commit `59c91a6`
[x] Auto-Update Mia (2026-09-14, mandiri) — autoUpdater.ts daily 04:00 WIB (AUTO_UPDATE_HOUR, 0=off): worktree-dirty check → git pull --ff-only → npm install → gates typecheck/test/verify → pushToOwner (TG+Discord) → restart tertunda 8s; state atomic `.data/auto-updater/state.json` (lock anti-double-run, dedup/hari); gate merah → ditolak + saran `git reset --hard`. Tools `auto_update_status` (read) + `auto_update` (write confirm). Commit `2de61ee`
```
...
```text
[x] Web chat + voice berfungsi (sudah)
[x] Telegram Bot berfungsi (chat text dari ponsel)  — @inimiaku_bot
...
[x] Reminder bisa disetel & dipicu (setidaknya di web; ideal via semua channel)  — remind_me + intent deterministik + push Telegram/Discord/web; model-authored variants pool (2026-09-07)
[x] Tool risky memerlukan konfirmasi di semua channel  — FR-014 inline ya/tidak di web/Telegram/Discord (+ automation headless auto-deny)
[x] Menambah channel baru tidak mengubah core  — channel adapter; core = runAssistantTurn
```
...
- [x] **White-box & playbooks** (adaptasi Strix, Apache-2.0): `sast_scan` (semgrep) + `security_playbook` (**84 pack** / 11 kategori: analysis, vulnerabilities ×33, tooling, protocols, frameworks, technologies, cloud, reconnaissance, custom, **methodology** (AppSec/OWASP 2025/API 2023/whitebox/fix-and-verify/source-aware/browser-transport-tampering/authenticated-testing), **scan_modes**) + `apps/web/security-playbooks/`
...
- [x] **Exploit Chain Builder (2026-09-19)**: `exploit_chain` 4 chain → **9 chain** (idor/auth_bypass/ssrf/session_fixation + race/graphql/xxe/open_redirect/cache_poison), batch koma-terpisah + `chainRunClaimSuffix` jujur
- [x] **Tier-1 Attack Suite (2026-09-20)**: `race_attack` (N-parallel + NONCE), `graphql_hunt` (introspection/suggestions/batching/depth), `cache_poison_prover` (host-header/fat-GET STRONG), `xxe_chain` (auto-OAST), `open_redirect_chain` (host-based verdict), `ws_hunt` (CSWSH + CDP tab), `github_osint` (commit-history secret scan), `har_import` (HAR → http_session)
- [x] **Business-logic fuzz (2026-09-20)**: `workflow_fuzz` (skip/repeat/reorder/value, CWE-840/841, 14 mutasi bounded)
- [x] **Recon sadar-obfuscasi (2026-09-21)**: `js_deobfuscate` (string-array/concat/source-map restore, eval-free) — Wajib setelah `js_mine` pada bundle besar/minified
- [x] **LLM prompt-injection hunt (2026-09-21)**: `prompt_injection_hunt` (delimiter/indirect/role-override, sinyal jujur)
- [x] **LLM red-team + MCP audit (2026-09-22)**: `llm_hunt` (jailbreak/rag/agency/exfil/pii → OWASP LLM Top 10 2025, baseline-controlled, canary-deterministik) + `mcp_hunt` (audit server MCP: initialize → tools/resources inventory, anon-access, sensitive-tool, arg-injection, resource scan) — CORE rebalance in-place (`security_scan`/`secret_scan` demote)`
- [x] **Honesty & delivery guards (2026-09-21)**: delivery guard (tool di luar jendela provider ditolak jujur, `toolCalls2` subset), TOOL BUDGET hint (+PENGECUALIAN PDF), `toolRunClaimSuffix` (klaim eksekusi tanpa bukti → catatan jujur), `pdfDeliverableSuffix` fabrikasi murni + `tryDeliverReportPdf`, `metaProse` (stage-direction) — CORE invariant runtime dikunci di verify.ts
...
- [x] **Chain composer + exploit artifact + session wizard + `csrf_prove`/`mass_assignment`/`exposure_hunt`/`upload_fuzz`/`xss_hunt`/`idor_enum`/`host_header_hunt`/`recon_full`/`smuggle_probe`/`dom_xss_prove` (2026-09-22 → 2026-09-23)**
...
- Pedoman & scope: `SECURITY.md`. **Total tools 312** · **CORE 128** · **84 playbook** · vitest 413.
