...
- **White-box & playbooks (2026-09-15, adaptasi Strix Apache-2.0)** — `sast_scan` (semgrep `p/default`+`p/secrets` di sandbox) + `security_playbook` (13 pack: counterevidence/severity-calibration/fix-verification/source-aware-sast/subdomain-takeover/oauth/graphql/llm-applications/nextjs/…) di `apps/web/security-playbooks/`. Prompt mewajibkan pass **counterevidence → severity-calibration → fix-verification** sebelum `finding_add`/`report`.
