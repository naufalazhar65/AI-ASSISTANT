...
- **White-box & playbooks (2026-09-15, adaptasi Strix Apache-2.0)** — `sast_scan` (semgrep `p/default`+`p/secrets` di sandbox) + `security_playbook` (68 pack di 9 kategori: analysis/custom/frameworks/protocols/reconnaissance/technologies/tooling/vulnerabilities/cloud — mis. counterevidence/severity-calibration/fix-verification, 26 kelas vuln (idor/ssrf/xss/sqli/ssti/xxe/race/…), tooling nmap/nuclei/ffuf/sqlmap/katana/naabu, oauth/graphql, aws/azure/gcp/kubernetes, nextjs/django/fastapi/nestjs) di `apps/web/security-playbooks/`. Prompt mewajibkan pass **counterevidence → severity-calibration → fix-verification** sebelum `finding_add`/`report`.
...
- **Cheat-sheet** — `SECURITY.md`. **Total tools 209.**
